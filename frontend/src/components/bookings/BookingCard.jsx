...
const BookingCard = ({ booking }) => {
    const { _id, customerId, vehicleId, rentedDate, returnedDate, totalRentalFee, rentalStatus } = booking;
        ...
        <div className="bg-white rounded-xl shadow-md p-6 flex flex-col gap-3 mb-6">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                ...
                <div>
                    ...
                    <p className="text-sm text-gray-500">Pickup date: {rentedDate ?? "N/A"}</p>
                    <p className="text-sm text-gray-500">Return date: {returnedDate ?? "N/A"}</p>
                    <p className="text-sm text-gray-500">Rental status: {rentalStatus ?? "N/A"}</p>
                    ...
                    <div className="flex flex-col justify-between">
                        <div className="text-sm flex flex-col gap-1">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="font-semibold">From {dayjs(rentedDate).format('D/M/YYYY')}</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <span className="w-2 h-2 rounded-full bg-purple-600" />
                                <span className="font-semibold">To {dayjs(returnedDate).format('D/M/YYYY')}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="flex flex-col items-end gap-2 w-full sm:w-auto">
                ...
                <div className="text-sm font-medium text-gray-800">Price: ${totalRentalFee}</div>
